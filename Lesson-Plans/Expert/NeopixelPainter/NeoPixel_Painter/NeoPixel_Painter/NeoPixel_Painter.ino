...
#define LED_PIN        8 // NeoPixels connect here
#define SPEED         A2 // Speed-setting dial
#define BRIGHTNESS    A2 // Brightness-setting dial
#define TRIGGER       5 // Playback trigger pin
...
void setup() {
    ...
    //digitalWrite(TRIGGER, HIGH);           // Enable pullup on trigger button
     // Poll startup trigger ASAP
    pinMode(TRIGGER, INPUT);
    Serial.begin(9600);
    startupTrigger=readTrigger();
    ...
    show();                                // Init LEDs to 'off' stated
#ifdef ENCODERSTEPS
    digitalWrite(5, HIGH);                 // Enable pullup on encoder pin
#endif

    Serial.print(F("Initializing SD card..."));
    if (!card.init(SPI_HALF_SPEED, CARD_SELECT)) {
   ...
   // Serial.println(F("ABC"));
    ...
    if (startupTrigger == 1) { // No button press
           Serial.println(F("startupTrigger == 1"));
        ...
        do {
            ...
            b = 255;
            found = bmpProcess(root, infile, NULL, &b);
            //Serial.print("Found: ");
           // Serial.println(found);
            if (found) { // b modified to safe max
                nFrames++;
       ...
       // brightnessLevel=analogRead(BRIGHTNESS);
        ...
        while (readTrigger() == 0); // Wait for button release
...
void loop() {
   ...
   // speedLevel=analogRead(SPEED);
    speedLevel=512;
    uint32_t linesPerSec = map(speedLevel, 0, 1023, 10, maxLPS);
    ...
    for (;;) {
        ...
        if (++block >= nBlocks) {                  // Past last block?
            //if (readTrigger() == 1) {      // Trigger released?
...
{
    uint16_t stTTrigger = digitalRead(TRIGGER);
    Serial.println(stTTrigger);
    if(stTTrigger==LOW)
    {
      return 1;
    }
